...
export default function Home() {
  return (
    <div className="animate-fade-up animate-ease-in animate-normal animate-fill-forwards bg-white animate-once animate-ease-linear px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-16 space-y-6 max-w-screen-md mx-auto">
      <div className="space-y-1">
        <h1 className="text-2xl font-semibold">Digital Zine</h1>
        <p className="text-sm text-gray-600">Designed by: Ken Fukutomi</p>
        <p className="text-sm text-gray-600">Tools: Figma, Illustrator, Excel</p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {["Figma", "Affinity Diagramming", "Data Setting", "Process Coding"].map((skill) => (
            <span
              key={skill}
              className="bg-black text-white rounded-full text-xs px-3 py-1 hover:bg-gray-700 ease-in cursor-default"
            >
              {skill}
            </span>
          ))}
